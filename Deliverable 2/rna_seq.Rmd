...
(common_lima <- intersect(LPS_limma, ctrl_limma))
```

### Significant genes for LPS group only
```{r}
setdiff(LPS_deseq, common_deseq)
setdiff(LPS_limma, common_lima)
```
...
setdiff(ctrl_limma, common_lima)
```

### Similarity Coefficients
```{r}
length(common_deseq) / length(union(LPS_deseq, ctrl_deseq))
length(common_lima) / length(union(LPS_limma, ctrl_limma))
```

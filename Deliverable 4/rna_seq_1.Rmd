...
mat = as.matrix(assay(se_immunedef,
                      "log_counts_cpm"))[rownames(results(dds)[order(results(dds)$padj),])[1:1000],] 
...
limma1000 <- rownames(topTable(fit, coef=ncol(design), number=1000))
```
...
C7_pathlist <- sapply(path_names, get_genes, sets) #3rd arg updated
...
```{r}
# rownames(topTable(fit, coef=ncol(design), number=1000)) # get the name of the top 1000 most significant pathways
# as.matrix(gsvaRes) # convert enrichment scores to a matrix structure
# All together: extract the enrichment scores of the top 1000 pathways and store that in a matrix

# Make a Heatmap of DEGs
mat = as.matrix(gsvaRes)[rownames(topTable(fit, coef=ncol(design), number=1000)),] 
# Using first 500 genes to simplify
...
results_1_df <- data.frame(deseq1000 = deseq1000,
                           limma1000 = limma1000) 
...
output_path <- fs::path(project_root,
                         relative_path, "data",
                         "NasalCVID_results_top1000.csv")
# write .csv file into Deliverable 4 folder

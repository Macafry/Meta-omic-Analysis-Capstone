...
if (!file.exists("data/translated_genes.txt")) {
...
# # get the name of the genes ordered by adjusted p-val
 ...
 top_genes = rownames(results(dds)[order(results(dds)$padj),])[1:1000]
# top_genes <- deseq1000 ...this didn't work either... although essentially same
...
# store the name of the top 1000 most significant genes, per Dr. Johnson
limma1000 <- rownames(topTable(fit, coef=2, number=1000))
```
...
# extract the log cpm of the top 1000 genes and store that in a matrix
# get the name of the top 1000 most significant genes
top_genes = rownames(topTable(fit, coef=2, number=1000))
...
enriched <- enrichr(deseq1000, dbs)
```
...
```{r}
enriched <- enrichr(limma1000, dbs)
```
...
# extract the log cpm of the top 1000 genes and store that in a matrix
# get the name of the top 1000 most significant genes, per Dr. Johnson
top_genes = rownames(topTable(fit, coef=2, number=1000))
# convert log cpm to a matrix structure
mat = as.matrix(assay(se_cvid_ctrl,"log_counts_cpm"))[top_genes,] 
...
ctrl_deseq <- deseq1000
ctrl_limma <- limma1000
...
results_2_df <- data.frame(ctrl_deseq=ctrl_deseq,
...
relative_path <- file.path("Meta-omic-Analysis-Capstone/Deliverable 4")
# file's output path
output_path <- fs::path(project_root,
                         relative_path, "data",
                         "PBMC_results_top1000.csv")
# write .csv into Deliverable 4 folder - 1000 top DGE comparison + presentation
write_csv(x=results_2_df, file=output_path)
